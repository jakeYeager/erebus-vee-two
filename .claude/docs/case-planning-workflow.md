...
1. An initial planning document will be made
2. It will be located at `topic-<identifier>/.claude/docs/planning-initial.md`
3. It will have a generalized hypothesis or purpose statement
4. It will use the previous topic `final-summary.md` as well as literature review to inform the purpose statement
5. It will have a generalized case testing suite to address or satisfy the purpose
...
Research will be conducted on existing academic publications that relate to, support or question what is described in Phase 1. In a concise, descriptive manner these academic references will be documented, for later reference at `topic-<identifier>/.claude/docs/academic-research.md`

## Phase 3 - Extend Refinement

A second "refinement" planning document will be created:
1. It will be located at `topic-<identifier>/.claude/docs/planning-refinement.md`
...
```
> **Status: Superseded.** See `topic-<identifier>/.claude/CLAUDE.md` for current case status.
```
...
The use-case for this CLAUDE.md is topic-level operational context. It should carry general information about the topic, and the essentials information for managing the current state of the cases. Update `topic-<identifier>/.claude/CLAUDE.md` with:
...
2. A case reference list — one line per case, with on-demand file link:
   ```
   Only read this file if you need [case title]: `topic-<identifier>/.claude/spec/case-<identifier>-spec.md`
   ```
...
1. It will be located at `topic-<identifier>/.claude/spec/case-<identifier>-spec.md`
...
4. The final required step of every spec — after all other deliverables are satisfied — must explicitly:
   - Update `topic-<identifier>/.claude/docs/topic-summary.md` with key results and final status
   - Update the case status in `topic-<identifier>/.claude/CLAUDE.md`
...
1. It will be created in the file located at `topic-<identifier>/.claude/docs/topic-summary.md`
...
4. Add a section to the root `README.md` for this topic listing all completed case whitepapers:
   ...
   - Link each entry to its `topic-<identifier>/output/case-<identifier>-whitepaper.md` file using the whitepaper's H1 title as the link text
