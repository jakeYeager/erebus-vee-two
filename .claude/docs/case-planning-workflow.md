...
2. It will be located at `topic-<identifier>/docs/planning-initial.md`
...
Research will be conducted on existing academic publications that relate to, support or question what is described in Phase 1. In a concise, descriptive manner these academic references will be documented, for later reference at `topic-<identifier>/docs/academic-research.md`

## Phase 3 - Extend Refinement

A second "refinement" planning document will be created:
1. It will be located at `topic-<identifier>/docs/planning-refinement.md`
...
```
> **Status: Superseded.** See `topic-<identifier>/CLAUDE.md` for current case status.
```
...
The use-case for this CLAUDE.md is topic-level operational context. It should carry general information about the topic, and the essentials information for managing the current state of the cases. Update `topic-<identifier>/CLAUDE.md` with:
...
2. A case reference list — one line per case, with on-demand file link:
   ```
   Only read this file if you need [case title]: `topic-<identifier>/spec/case-<identifier>-spec.md`
   ```
...
1. It will be located at `topic-<identifier>/spec/case-<identifier>-spec.md`
...
4. The final required step of every spec — after all other deliverables are satisfied — must explicitly:
   - Update `topic-<identifier>/docs/topic-summary.md` with key results and final status
   - Update the case status in `topic-<identifier>/CLAUDE.md`
...
1. It will be created in the file located at `topic-<identifier>/docs/topic-summary.md`
